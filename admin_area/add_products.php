...
if(isset($_POST['addProduct_submit'])){
  ...
  if(empty($productTitle) OR empty($productKeywords) OR empty($productCategories) OR empty($productPrice) OR empty($productStatus)){
    echo "<script>alert('Please fill all the fields available')</script>";
    exit();
  }
...
<header>
  ...
  <!--Bootstrap Navbar ends here-->
    ...
    <div class="form-outline mb-4 w-50 m-auto">
      ...
      <input type="text" name="product_price" id="product_price" class="form-control" placeholder="Enter Price" autocomplete="off">
    </div>
